...
app.get("/api/users/get", async (req, res) => {
    ...
    if (isAdmin == null)
        ...
        return res
            .status(400)
            .json({ error: "User not authorized to make this request" });
...
app.get("/api/ads/get/:category/author/:author", async (req, res) => {
    ...
    const AdModel = categoryModelMap[req.params.category];
    try {
        const ads = await AdModel.find({ author: req.params.author }).sort({
            postDate: "asc",
            title: "asc",
        });
        res.status(200).json(ads);
...
app.patch("/api/ads/update/:category/id/:id", async (req, res) => {
    ...
    // if user not admin, check if they are updating their own post
        ...
        if (ad != null && ad.author != result.username)
            return res.status(400).json({
                error:
                    "User not admin or not authorized to make this request on another users post",
            });
    }
...
app.get("/api/chats/get/:user1/:user2", async (req, res) => {
    try {
        const chat = await Chat.findOne({
            $or: [
                {
                    $and: [{ user1: req.params.user1 }, { user2: req.params.user2 }],
                },
                {
                    $and: [{ user1: req.params.user2 }, { user2: req.params.user1 }],
                },
            ],
        }).select(["messages"]);
...
app.get("/api/chats/get/:username", async (req, res) => {
    try {
        const chats = await Chat.find({
            $or: [{ user1: req.params.username }, { user2: req.params.username }],
        }).select(["user1", "user2"]);
